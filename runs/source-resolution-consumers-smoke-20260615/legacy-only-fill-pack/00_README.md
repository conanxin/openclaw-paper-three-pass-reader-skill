...
| --- | --- |
| slug | `runs` |
| paper_reading.json path | `runs/source-resolution-consumers-smoke-20260615/legacy-only-paper_reading.json` |
...
- **Match type**: `title`
- **Confidence**: `high`
- **Matched paper**: `how-to-read-a-paper`
- **Matched arXiv ID**: `(none)`
- **Matched repo**: `(none)`
- **Resolver source**: `(none)`
- **Source resolution step**: `cli overlay via p3pr paper_title subcommand; input='How to Read a Paper'`
- **Candidate count**: `0`
- **Structured trail**: `False`
...
python3 skills/paper-three-pass-reader/scripts/render_page.py \
  --input runs/source-resolution-consumers-smoke-20260615/legacy-only-paper_reading.json \
...
python3 skills/paper-three-pass-reader/scripts/audit_paper_reading.py \
  --input runs/source-resolution-consumers-smoke-20260615/legacy-only-paper_reading.json --json-output runs/source-resolution-consumers-smoke-20260615/audit_result.json
```
...
> but the page's reading mode badge must show `partial_text` honestly.
