# 06. Pass 3 — Method Reconstruction

## Goal

Reconstruct the method in your own words: input → algorithm → output. A non-reader should be able to retell it.
> **Weak-input note**: `reading_mode = abstract_only`. Pass 3 method reconstruction CANNOT be filled. Keep `[DRAFT]` and acknowledge in audit that it is `unavailable_due_to_reading_mode`.


## Allowed materials

- full_text: method / algorithm / equations / appendix.
- weak: abstract range only.

## Forbidden

- Don't claim to have reconstructed details that came from body you didn't read.

## JSON fields to fill

- `pass3.method_reconstruction`
- `pass3.hidden_assumptions`

## Evidence label rules

Default label: [Agent inference] with reasoning. weak mode: leave [DRAFT].

## Output format

full_text: method_reconstruction has at least 3 steps. weak: [DRAFT] remains, with notes explaining.

## Stop condition

Reconstruction length adequate for the reading mode.
