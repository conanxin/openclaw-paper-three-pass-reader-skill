...
| --- | --- |
| slug | `runs` |
| paper_reading.json path | `runs/source-resolution-consumers-smoke-20260615/degraded-paper_reading.json` |
...
- **Hint input**: `Some Unknown Paper`
- **Resolver status**: `error`
- **Match type**: `(none)`
- **Confidence**: `low`
- **Matched paper**: `(none)`
- **Matched arXiv ID**: `(none)`
- **Matched repo**: `(none)`
- **Resolver source**: `skills/paper-three-pass-reader/data/resolver_hints.json`
- **Source resolution step**: `hostile-resolver`
- **Candidate count**: `0`
- **Structured trail**: `True`
- **Legacy fallback**: `False`
- **Degraded fallback**: `degraded = ambiguous_clue` — paper identity is not confirmed; ask the user to verify before continuing.
...
python3 skills/paper-three-pass-reader/scripts/render_page.py \
  --input runs/source-resolution-consumers-smoke-20260615/degraded-paper_reading.json \
...
python3 skills/paper-three-pass-reader/scripts/audit_paper_reading.py \
  --input runs/source-resolution-consumers-smoke-20260615/degraded-paper_reading.json --json-output runs/source-resolution-consumers-smoke-20260615/audit_result.json
```
...
> but the page's reading mode badge must show `abstract_only` honestly.
