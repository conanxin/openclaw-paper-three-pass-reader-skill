...
| `v0.2.14-alpha` | 当前 | `p3pr url <url>` 子命令。直接抓取用户给定的 HTML 网页(或 PDF),用 stdlib `html.parser` 做纯文本抽取,把抽取结果作为 `input_kind=paper_url` 喂给 runner,用 `--input-file` + `--paper-url`。`p3pr.py` 新增 `_HTMLTextExtractor` 与 `_fetch_url()`。runner 现在允许同时传 `--input` 与 `--input-file`(审计串 + 正文)。reading_mode 纪律:HTML ≥ 800 字符 → `full_text`,否则 `partial_text`;无正文的 PDF 保持 `partial_text`。新增 `P3PR_SOURCE_URL:` 摘要行,新增 CLI `--authors` 与 `--year` 旗标。验证 261/0 PASS。实站 URL 烟测页 `you-and-your-research-url-smoke-cn` 已发布,实站 audit `pages=11 pass=11 warn=0 fail=0`。 |
...
版本:**v0.2.14-alpha**。前序:v0.2.13-alpha (不可变,保留以保证可复现)。
