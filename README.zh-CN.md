...
| `v0.2.12-alpha` | 不可变 | 根首页审计豁免。新增 `page_type` 分类(`site_index` / `paper_page` / `manifest` / `unknown`),根首页不再被论文级检查(`missing_resolver_trail` / `missing_claims_section` / `missing_glossary`)误报。严重检查(`template_leak` / `raw_dict` / `old_footer`)在根首页仍生效。新增顶层 `page_type_counts` 与每页 `page_type` 字段;Markdown 报告新增 `## Page Type Summary` 段。新增 `--include-manifest` 旗标。`scripts/validate.sh` 第 18 步用 8 个 selftest fixture + 实站点端到端校验。验证 236/0 PASS。 |
| `v0.2.13-alpha` | 不可变 | 根首页 manifest 链接。`publish_output_to_github.sh` 在 `<head>` 输出 `<link rel="alternate" type="application/json" href="published_pages.json">`,并在 About 区输出可见 `<a href="published_pages.json">` 链接(中英双语标签)。审计的 `_check_site_index()` 接受这两种形式。实站 audit 最后一个 info 提示 (`index_no_manifest_link`) 已消失。验证 242/0 PASS。 |
| `v0.2.14-alpha` | 不可变 | `p3pr url <url>` 子命令。直接抓取用户给定的 HTML 网页(或 PDF),用 stdlib `html.parser` 做纯文本抽取,把抽取结果作为 `input_kind=paper_url` 喂给 runner,用 `--input-file` + `--paper-url`。`p3pr.py` 新增 `_HTMLTextExtractor` 与 `_fetch_url()`。runner 现在允许同时传 `--input` 与 `--input-file`(审计串 + 正文)。reading_mode 纪律:HTML ≥ 800 字符 → `full_text`,否则 `partial_text`;无正文的 PDF 保持 `partial_text`。新增 `P3PR_SOURCE_URL:` 摘要行,新增 CLI `--authors` 与 `--year` 旗标。验证 261/0 PASS。实站 URL 烟测页 `you-and-your-research-url-smoke-cn` 已发布,实站 audit `pages=11 pass=11 warn=0 fail=0`。 |
| `v0.2.15-alpha` | 不可变 | 修复 `p3pr --publish` 在缺少 `paper-reading-output/index.html` 时仍推 404 stub 的 bug。v0.2.15 dogfood 阶段实测:runner 正确跳过 render(audit/qg FAILED)但 `p3pr.py` 仍调用 publisher,推了一个空目录到 gh-pages。新版本在缺少 `index.html` 时直接硬 BLOCK,即便传了 `--allow-draft-publish`。新增第 20l 步验证子项。v0.2.15 dogfood 留下的 404 stub 与对应 manifest 条目已从 `gh-pages` 清理。验证 263/0 PASS。 |
| `v0.3.0` | 当前 | 首个 **stable** 版本。无新功能。从 `v0.3.0-alpha` 起的 delta:1 个 housekeeping commit (`cfa2b80`) 提交了 4 个 modified 文件(v0.2.10 release 后 doc polish + v0.2.18 finalize smoke refresh + 2 个 v0.2.15 fill-pack snapshot 重存),并把 21 个 untracked run 目录加入 `.gitignore`(`runs/p3pr-cli-*`、`runs/quality-gate-smoke-20260615`、`runs/second-me-zh-cn-20260615`、`runs/you-and-your-research-20260615/{REPORT.md,fill-pack,work,you-and-your-research-cn/{input,reports,work/audit_result.json}}` 等)。1 个 follow-up commit (`c5f6efa`) stage 了 final cleanroom 产物(3 个 doctor JSON + 1 个 status JSON + 1 个 published_pages_audit.json+.md + 3 个 dry-run logs)。`PHASE_P3PR_V0_3_0_CLEANROOM_HOUSEKEEPING_REPORT.md` 记录了完整 delta。`STABLE_CLEANROOM_CHECKLIST.md` 记录了 clean working tree 上的 readiness:validation 305/0、doctor 25/0/0、live audit 14/14。`v0.3.0-alpha` 移到 `previous`。所有 v0.2.x / v0.3.0-alpha run 目录与已发布 Page 仍然兼容。无旧 tag 移动、无 force-push、无旧 release 删除。 |
| `v0.3.0-alpha` | 前序 | 首个 **稳定候选版本 (stable-readiness RC)**。无新功能。文档:`STABLE_READINESS_CHECKLIST.md`、`RELEASE_NOTES_v0.3.0-alpha.md`、`PHASE_P3PR_V0_3_0_STABLE_READINESS_REPORT.md`。README.md / README.zh-CN.md Quick Start 同步展示两阶段流程 + `status` / `doctor` 管理 + 站点审计。Bug 修复:`p3pr doctor` summary 计数现在正确小写化 check status(之前因为用大写 key 查表,counter 始终是 0/0/0)。验证 305/0 PASS。实站 audit 14/14 PASS, 0 warn, 0 fail。doctor 24/1/0(1 WARN 是 dirty working tree,发布过程中属正常)。**这不是** v0.3.0 stable,见 `STABLE_READINESS_CHECKLIST.md` §"Not yet stable"。 |
| `v0.2.19-alpha` | 前序 | 新增 `p3pr status` 与 `p3pr doctor` —— 只读的可观测性子命令。`status` 扫描 `runs/` 并读取 `published_pages.json`,把每个 run 归类为 `draft` / `filled` / `audited` / `rendered` / `rendered_with_warnings` / `published` / `blocked` / `unknown`,并交叉对照 manifest 标记 `published`。`doctor` 跑 7 组健康检查:本地环境、必备脚本、必备文档、git 状态、gh CLI / auth、可选 `validate.sh`、轻量 HEAD 探活。两者只读,都支持 `--json-output`,都打印固定 `P3PR_STATUS_*` / `P3PR_DOCTOR_*` 摘要块。dirty working tree 与未登录的 gh 都是 WARN,绝非 FAIL。新增 `STATUS_AND_DOCTOR.md`。新增第 23 步验证,12 个子项。验证 305/0 PASS。 |
| `v0.2.18-alpha` | 前序 | `p3pr finalize <run-dir>` UX 打磨。自动从 `paper_reading.json` 推断 gh-pages site-path 与发布页 page-title(显式 `--site-path` / `--page-title` 仍可覆盖)。摘要块新增 `P3PR_SITE_PATH` / `P3PR_PAGE_TITLE` / `P3PR_READING_MODE` / `P3PR_LANGUAGE` / `P3PR_AUDIT_STATUS` / `P3PR_QUALITY_GATE_STATUS` / `P3PR_WARNING_COUNT` / `P3PR_WARNING_SUMMARY`(列出至多 3 条实际 warning,不是泛泛 "warnings exist")。`P3PR_NEXT_ACTION` 现在按状态给出具体指引。`--dry-run` 新增 `inferred_site_path` / `inferred_page_title` 与来源说明。v0.2.15 / v0.2.17 publish guard 全部保留(由新增第 22 步验证)。新增第 22 步验证,14 个子项。实站 dogfood 页 `https://conanxin.github.io/paper-reading-pages/you-and-your-research-url-finalize-ux-cn/`。验证 293/0 PASS。 |
| `v0.2.17-alpha` | 前序 | 新增 `p3pr finalize <run-dir>` —— 第二阶段 CLI。读取 `<run-dir>/work/paper_reading.json` 后顺序执行:audit → zh-CN quality gate → render → 可选 publish → 可选 published-pages audit。每次退出均输出固定 `P3PR_FINALIZE_STATUS` 摘要块。`--dry-run` 模式打印 `P3PR_FINALIZE_DRY_RUN` 计划块。继承 v0.2.15 publish-gate:render 之后若 `paper-reading-output/index.html` 缺失则 BLOCK。旗标:`--publish` / `--no-publish` / `--repo` / `--branch` / `--site-path` / `--page-title` / `--allow-warnings` / `--allow-draft-publish` / `--skip-quality-gate` / `--skip-published-audit` / `--dry-run`。新增第 21 步验证,16 个子项。实站 dogfood 页 `https://conanxin.github.io/paper-reading-pages/you-and-your-research-url-dogfood-finalize-cn/`。验证 279/0 PASS。 |
...
版本:**v0.3.0**。前序:v0.3.0-alpha (不可变,保留以保证可复现)。
