...
| `v0.3.0` | **deferred** | 稳定版 v0.3.0 **deferred**,见 `PHASE_P3PR_V0_3_0_STABLE_CLEANROOM_REPORT.md`。cleanroom 其他部分均通过(验证 305/0 PASS、实站 audit 14/14 PASS、doctor 24/1/0、3 个 dry-run 烟测均通过、gh auth OK),但 `p3pr doctor` 因历史遗留 dirty working tree 报 1 个 WARN:4 个 modified 文件 + 21 个 untracked 目录都来自 v0.2.x 与 v0.3.0-alpha 阶段的运行产物。本阶段 spec 要求 "stable 发布前最好无 WARN" 且明确禁止删未知 dirty 文件来消除 WARN。用户可任选:把历史遗留作为 housekeeping 一次性 commit、把 `runs/` 目录加入 `.gitignore`、或接受 `PASS_WITH_WARNINGS` 直接发布。三种方案都不会动旧 tag、不会 force-push、不会删旧 release。**没有创建 v0.3.0 tag**;`v0.3.0-alpha` 仍是最新 release。 |
| `v0.3.0-alpha` | 当前 | 首个 **稳定候选版本 (stable-readiness RC)**。无新功能。文档:`STABLE_READINESS_CHECKLIST.md`、`RELEASE_NOTES_v0.3.0-alpha.md`、`PHASE_P3PR_V0_3_0_STABLE_READINESS_REPORT.md`。README.md / README.zh-CN.md Quick Start 同步展示两阶段流程 + `status` / `doctor` 管理 + 站点审计。Bug 修复:`p3pr doctor` summary 计数现在正确小写化 check status(之前因为用大写 key 查表,counter 始终是 0/0/0)。验证 305/0 PASS。实站 audit 14/14 PASS, 0 warn, 0 fail。doctor 24/1/0(1 WARN 是 dirty working tree,发布过程中属正常)。**这不是** v0.3.0 stable,见 `STABLE_READINESS_CHECKLIST.md` §"Not yet stable"。 |
...
版本:**v0.3.0-alpha**。前序:v0.2.19-alpha (不可变,保留以保证可复现)。
