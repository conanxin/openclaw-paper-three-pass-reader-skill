...
| `v0.2.10-alpha` | 当前 | 已发布页面回归审计。新增 `audit_published_pages.py` 读取 `published_pages.json`,抓取每页,生成 JSON + Markdown 审计报告,覆盖 template leak、raw dict、旧 footer、弱 zh-CN UI、缺 Resolver Trail、缺 Claims / Glossary、essay 模式回归。`--selftest-dir` 模式接入 `scripts/validate.sh` 第 17 步。首次实测审计 1/9 页面 PASS。验证 225/0 PASS。 |
...
版本:**v0.2.10-alpha**。前序:v0.2.9-alpha (不可变,保留以保证可复现)。
