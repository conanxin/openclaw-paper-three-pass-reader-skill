...
| `v0.2.9-alpha` | 当前 | 打磨 HTML 随笔 / 演讲型页面渲染。renderer 不再泄漏原始 `{'label': ...}` 字典,不再泄漏 `{% else %}` 模板标签,页面 footer 暴露 `generator_version`,essay 模式下 `Reproduction Plan` 切换为 `实践计划`,并使用 `<details>` 实现的折叠面板。Claim ID 与术语表定义正确显示。重新发布 *You and Your Research* 中文页面。验证 220/0 PASS。 |
...
版本:**v0.2.9-alpha**。前序:v0.2.8-alpha (不可变,保留以保证可复现)。
