...
| `v0.2.7-alpha` | 当前 | v0.2.6 round-2 硬化,以新 tag 发布 (v0.2.6-alpha 不可变,绝不重指)。新增结构化顶层 `source_resolution`、CLI→runner overlay (`work/resolver_source.json` + `--resolver-source`)、resolver 助手降级行为(助手坏了降为 `ambiguous_clue` 但 run 仍 rc=0 完成)。验证 195/0 PASS。 |
...
版本:**v0.2.7-alpha**。前序:v0.2.6-alpha (不可变,保留以保证可复现)。
