...
| `v0.2.13-alpha` | 当前 | 根首页 manifest 链接。`publish_output_to_github.sh` 在 `<head>` 输出 `<link rel="alternate" type="application/json" href="published_pages.json">`,并在 About 区输出可见 `<a href="published_pages.json">` 链接(中英双语标签)。审计的 `_check_site_index()` 接受这两种形式。实站 audit 最后一个 info 提示 (`index_no_manifest_link`) 已消失。验证 242/0 PASS。 |
...
版本:**v0.2.13-alpha**。前序:v0.2.12-alpha (不可变,保留以保证可复现)。
