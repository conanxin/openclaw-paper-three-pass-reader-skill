...
| `v0.3.0` | current | First **stable** release of `paper-three-pass-reader`. No new features. Delta from `v0.3.0-alpha`: a single housekeeping commit (`cfa2b80`) commits the 4 tracked-modified files (v0.2.10 post-release doc polish + v0.2.18 finalize smoke refresh + 2 v0.2.15 fill-pack snapshot re-saves) and adds 21 untracked run dirs to `.gitignore` (`runs/p3pr-cli-*`, `runs/quality-gate-smoke-20260615`, `runs/second-me-zh-cn-20260615`, `runs/you-and-your-research-20260615/{REPORT.md,fill-pack,work,you-and-your-research-cn/{input,reports,work/audit_result.json}}`, etc.). A follow-up commit (`c5f6efa`) stages the final cleanroom artifacts (3 doctor JSONs + 1 status JSON + 1 published_pages_audit.json+.md + 3 dry-run logs). `PHASE_P3PR_V0_3_0_CLEANROOM_HOUSEKEEPING_REPORT.md` documents the full delta. `STABLE_CLEANROOM_CHECKLIST.md` records the readiness check on a clean working tree: validation 305/0, doctor 25/0/0, live audit 14/14. `v0.3.0-alpha` moves to `previous`. All v0.2.x / v0.3.0-alpha run directories and existing Pages remain compatible. No old tags moved, no force pushes, no old releases deleted. |
