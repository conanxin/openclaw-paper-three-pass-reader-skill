...
| `v0.3.0` | **deferred** | Stable release **deferred** in `PHASE_P3PR_V0_3_0_STABLE_CLEANROOM_REPORT.md`. The cleanroom is otherwise fully clean (validation 305/0 PASS, live audit 14/14 PASS, doctor 24/1/0, all 3 dry-run smokes pass, gh auth OK), but `p3pr doctor` reports a `git_working_tree` WARN because of a historical backlog of 4 modified files + 21 untracked run dirs from prior phases. The spec for the cleanroom phase calls for "no WARN" before a stable release, and explicitly forbids deleting unknown dirty files or force-cleaning to suppress the WARN. The user can review the dirty files and either commit the historical backlog as housekeeping, add the run dirs to `.gitignore`, or accept `PASS_WITH_WARNINGS` and ship anyway. See the phase report for the three options. No v0.3.0 tag was created; `v0.3.0-alpha` remains the latest released. |
