...
PYEOF
    ...
    # v0.2.13-alpha: include a manifest link so tools (and the audit) can
    # discover published_pages.json from the index page itself. The link is
    # emitted in two forms:
    #   1. <head> <link rel="alternate" type="application/json" href="published_pages.json">
    #      — machine-readable manifest discovery.
    #   2. A visible <a href="published_pages.json"> entry in the About section.
    #      — human-visible, with English + Chinese labels.
    # Idempotency: if either form already exists, the second emit is a no-op
    # duplicate. We do not actively strip duplicates because the file is fully
    # regenerated on every publish and the goal is a stable, correct page.
